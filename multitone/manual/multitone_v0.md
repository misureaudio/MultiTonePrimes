...
Why primes? If two tones at frequencies *f₁* and *f₂* are harmonically related (e.g., *f₂* = 2·*f₁*), then their intermodulation products (sum and difference frequencies) fall back on the same harmonic grid, reinforcing each other and creating localized peaks. Primes are, by definition, not multiples of each other, so intermodulation products are distributed across the spectrum, producing a flatter overall frequency response — the hallmark of a good test signal.
